...
namespace MinJSON.Serialization
{
    public class JsonDeserializer<T>
    {
        ...
        private IDictionary DeserializeDictionary(JsonValue jvalue, JsonPropertyAttribute propertyAttribute, Type instance_type)
        {
            ...
            Type keyType = instance_type.GetGenericArguments()[0];
            Type valType = instance_type.GetGenericArguments()[1];
        ...
        {
            JsonSerializable serializable = classType.GetCustomAttribute(typeof(JsonSerializable)) as JsonSerializable;
            if (serializable == null)
            {
                throw new SerializationException("The target class is not marked as JsonSerializable");
            }
